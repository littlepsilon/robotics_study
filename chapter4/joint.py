...
class PrismaticJoint(Joint):
    def __init__(self, name, position, translation_axis=[0, 0, 1], orientation=np.eye(3)):
        """
        Joint allowing translational motion along the specified translation axis.
        """
        super().__init__(name, position, orientation)
    ...
    def compute_screw_axis(self, reference_frame=None):
        """
        Computes the screw axis for a prismatic joint.
        If a reference frame is provided, the axis is transformed accordingly.
        """
